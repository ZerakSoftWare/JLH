...
+(NSMutableArray*)IndexArray:(NSArray*)stringArr
{
    ...
    {
        NSString *pinyin = ((ChineseString*)object).pinYin;
        if (VV_IS_NIL(pinyin) ) {
            pinyin = @"#";
        }else{
...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr
{
    ...
    for(int i=0;i<[stringArr count];i++)
    {
        ...
        NSDictionary *dic = [stringArr objectAtIndex:i];
        
        chineseString.cityCode = [NSString stringWithFormat:@"%@",dic[@"CityCode"]];
        
        chineseString.string=[NSString stringWithString:dic[@"CityName"]];
       ...
       // VVLog(@"string====%@",chineseString.string);
...
+(NSMutableArray*)SortArray:(NSArray*)stringArr
{
    ...
    for(int i=0;i<[stringArr count];i++){
        ...
        VVLog(@"SortArray----->%@",((ChineseString*)[tempArray objectAtIndex:i]).string);
    }
    return result;
}
@end
